  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
      ...
      <div style={{
        ...
        background: focused ? "linear-gradient(135deg, var(--accent-purple), var(--accent-blue))" : "rgba(255,255,255,0.08)",
        ...
        <input id={id} type={type} value={value} onChange={onChange}
          ...
          style={{
            width: "100%", background: "rgba(5,8,22,0.85)", border: "none", borderRadius: 13,
            padding: "13px 16px", color: "var(--text)", fontSize: 14.5,
...
function Divider({ label = "or" }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
      ...
      <span className="mono" style={{ fontSize: 11, color: "var(--text-faint)", letterSpacing: 0.5 }}>{label}</span>
      <div style={{ flex: 1, height: 1, background: "rgba(255,255,255,0.08)" }} />
    </div>
  );
}
...
function PasswordStrength({ password }) {
  ...
  const colors = ["var(--danger)", "#FB923C", "var(--success)"];
    ...
    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", gap: 10 }}>
          {checks.map((c) => (
            <span key={c.label} style={{ fontSize: 11, color: c.pass ? "var(--success)" : "var(--text-faint)", transition: "color 0.2s" }}>
              {c.pass ? "✓" : "○"} {c.label}
            </span>
          ))}
        </div>
        {score > 0 && (
          <span className="mono" style={{ fontSize: 10.5, color: colors[score - 1], letterSpacing: 0.4 }}>
            {labels[score - 1]}
          </span>
        )}
      </div>
...
export function Signup() {
  ...
  const canSubmit = email.trim() && password.length >= 6 && password === confirm && !loading && !googleLoading;
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      toast.push("Account created. Welcome to ObjectionAI!", "success");
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      toast.push("Signed up with Google!", "success");
      navigate("/", { replace: true });
    } catch (err) {
      const msg = FirebaseErrorMessage(err.code);
  ...
  return (
    <div style={{
      ...
      <div style={{
        position: "fixed", top: "8%", right: "18%", width: 380, height: 380,
        borderRadius: "50%", background: "radial-gradient(circle, rgba(139,92,246,0.22), transparent 70%)",
        filter: "blur(60px)", pointerEvents: "none",
      }} />
      <div style={{
        position: "fixed", bottom: "10%", left: "12%", width: 300, height: 300,
        ...
        >
          <div>
            ...
            <p style={{ fontSize: 14, color: "var(--text-secondary)", margin: 0, lineHeight: 1.5 }}>
              Join ObjectionAI and turn objections into opportunities.
            </p>
          </div>
          ...
          <motion.button
            ...
            style={{
              ...
              gap: 10, padding: "12px 20px", borderRadius: 14,
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            {/* confirm password with live border feedback */}
            <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              <label htmlFor="confirm" className="mono"
                style={{ fontSize: 11, letterSpacing: 0.5, color: "var(--text-faint)", textTransform: "uppercase" }}>
                Confirm Password
              </label>
              <div style={{
                ...
                <input id="confirm" type="password" value={confirm}
                  ...
                  style={{
                    width: "100%", background: "rgba(5,8,22,0.85)", border: "none",
                    borderRadius: 13, padding: "13px 16px", color: "var(--text)",
            ...
            <motion.button
              ...
              style={{
                marginTop: 2, padding: "13px 20px", borderRadius: 14,
          ...
          <div style={{ textAlign: "center" }}>
            <span style={{ fontSize: 13.5, color: "var(--text-secondary)" }}>Already have an account? </span>
            <Link to="/login" style={{ fontSize: 13.5, fontWeight: 600, color: "var(--accent-purple)", textDecoration: "none" }}>
              Sign in
            </Link>
          </div>
        </motion.div>
